...
We will now investigate how much a certain type of alcohol sold impacts alcoholism levels. We will create a regressive model where male and female are factors. We'll start with the effect of just spirits, or hard alcohol, to investigate if there is a relation between the proportion of alcohol sales in a country attributed to spirits and the prevalence of alcoholism. 
...
proportion <- c(alcohol$Spirits, alcohol$Spirits)
responses <- c(alcoholism$Male, alcoholism$Female)
factors <- c(rep("M", length(alcoholism$Male)), rep("F", length(alcoholism$Female)))

gender <- data.frame(countries2, responses, proportion, factors)
colnames(gender) <- c("country", "response", "Spirits", "factor")
gender <- gender[,-1]

head(gender)
```

The above is the first few rows of a data table containing the alcoholism prevalence levels of a specific country for a gender and the proportion of the countries alcohol sale that are spirits. 
...
scatterplot(gender$response ~ gender$Spirits + gender$factor,
            ...
            main = "Relation between alcoholism prevalence and spirits by country")
...
```{r echo = TRUE}
spirits.lm <- second <- lm(response ~ proportion + factor, data = gender)
